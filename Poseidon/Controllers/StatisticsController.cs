...
namespace Poseidon.Controllers
{
    ...
    public class StatisticsController : ControllerBase
    {
        ...
        /// <summary>
        /// Retrieves the number of passengers who survived.
        /// </summary>
        /// <returns>The count of passengers who survived.</returns>
        [HttpGet("survivors")]
        [Authorize(Roles = "User, Admin")]
        [SwaggerOperation(Summary = "Get number of survivors", Description = "Returns the number of passengers who survived.")]
        public async Task<IActionResult> GetNumberOfSurvivors()
        {
            var survivorsCount = await _passengerService.GetNumberOfSurvivorsAsync();
            return Ok(new { NumberOfSurvivors = survivorsCount });
        }
        ...
        /// <summary>
        /// Retrieves the number of passengers within a specified age range.
        /// </summary>
        /// <param name="minAge">The minimum age of the range.</param>
        /// <param name="maxAge">The maximum age of the range.</param>
        /// <returns>The count of passengers within the specified age range.</returns>
        [HttpGet("age-range")]
        [Authorize(Roles = "User, Admin")]
        [SwaggerOperation(Summary = "Get number of passengers by age range", Description = "Returns the number of passengers within the specified age range.")]
        public async Task<IActionResult> GetNumberOfPassengersByAgeRange([FromQuery] double minAge, [FromQuery] double maxAge)
        {
            ...
            var passengers = await _passengerService.GetPassengersByAgeRangeAsync(minAge, maxAge);
            var count = passengers.Count();
            return Ok(new { NumberOfPassengers = count });
        }

        /// <summary>
        /// Retrieves the number of passengers within a specified fare range.
        /// </summary>
        /// <param name="minFare">The minimum fare of the range.</param>
        /// <param name="maxFare">The maximum fare of the range.</param>
        /// <returns>The count of passengers within the specified fare range.</returns>
        [HttpGet("fare-range")]
        [Authorize(Roles = "User, Admin")]
        [SwaggerOperation(Summary = "Get number of passengers by fare range", Description = "Returns the number of passengers within the specified fare range.")]
        public async Task<IActionResult> GetNumberOfPassengersByFareRange([FromQuery] double minFare, [FromQuery] double maxFare)
        {
            if (minFare < 0 || maxFare < minFare)
            {
                return BadRequest("Invalid fare range.");
            }

            var passengers = await _passengerService.GetPassengersByFareRangeAsync(minFare, maxFare);
            var count = passengers.Count();
            return Ok(new { NumberOfPassengers = count });
        }
